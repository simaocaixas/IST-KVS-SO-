...
int parse_file(int fd_in, int fd_out, const char *dir_name, char *file_name,
               int total_backups) {
  ...
  while (1) {
    ...
    const char *help_string = "Available commands:\n"
    ...
    case CMD_WRITE:
      num_pairs =
          parse_write(fd_in, keys, values, MAX_WRITE_SIZE, MAX_STRING_SIZE);
      if (num_pairs == 0) {
    ...
    case CMD_DELETE:
      num_pairs =
          parse_read_delete(fd_in, keys, MAX_WRITE_SIZE, MAX_STRING_SIZE);
    ...
    case CMD_WAIT:
      ...
      if (delay > 0) {
        ...
        while (total_written < lineSize) {
          ssize_t written =
              write(fd_out, line + total_written, lineSize - total_written);
          if (written < 0) {
...
/**
 * Generates input and output file paths for processing files with a ".job"
 * extension.
 *
 ...
 * @return 1 if the paths were successfully generated for a ".job" file, 0
 * otherwise.
 */
int generate_paths(const char *dir_name, struct dirent *dir_entry,
  ...
  if (snprintf(in_path, MAX_JOB_FILE_NAME_SIZE, "%s/%s", dir_name, file_name) >=
  ...
  if (out_ext) {
    strncpy(out_ext, ".out",
            (size_t)(MAX_JOB_FILE_NAME_SIZE - (out_ext - out_path)));
  }
 ...
 * Reads file entries, generates input/output paths, and processes files with a
 * ".job" extension.
 *
 * @param arg A `void*` argument pointing to the directory name (expected to be
 * `const char*`).
 * @return NULL on completion. This function is designed to be used as a thread
 * entry point.
 */
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < max_threads; i++) {
    ...
    if (pthread_create(&threads[i], NULL, process_file, (void *)dir_name) !=
        0) {
